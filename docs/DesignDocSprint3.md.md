...
* **`OrganizationController`** --- REST controller mapped to the `/organization`, handling the ResponseEntity requests to create, delete, and edit organizations by adding and deleting needs. This controller functions similar to `FundingBasketController`.
* **`ProposalController`** --- REST controller which handles the CRUD operations for the proposals created by users. Additionally, it handles the logic of Approving or Rejecting proposals, as well as updating the amount of votes a proposal has
* 
...
* **`OrganizationService`** --- Spring `@Service` that implements the Organization operations. Provides CRUD functionality to Organizations.
* **`ProposalService`** --- Spring `@Service` that implements the Proposal operations. Provides CRUD functionality to Proposals.
...
* **`FundingBasketFileDAO`** — `@Repository` implementation that persists FundingBaskets to a JSON file (`data/fundingbaskets.json`) using a Jackson `ObjectMapper`. On startup it loads the file into an in-memory `Map` cache; reads (such as `getFundingBasketArray()`) are served from the cache, and writes are saved back to disk
* **`AccountDAO`** — interface defining the persistence contract (`getAccount`, `createAccount`, `addAllAccounts`)
* **`AccountFileDAO`** — `@Repository` implementation that persists accounts to a JSON file (`data/accounts.json`) using a Jackson `ObjectMapper`. On startup it loads the file into an in-memory `Map` cache; reads (such as `getAccountArray()`) are served from the cache, and writes are saved back to disk
* **`OrganizaitonDAO`** — interface defining the persistence contract (`getOrganization`, `getOrganizationArray`, `createOrganization`, `updateOrganization`, `deleteOrganization`, `addNeed`, `deleteNeed`)
* **`OrganizationFileDAO`** `@Repository` implementation that persists organizations to a JSON file (`data/organizations.json`) using a Jackson `ObjectMapper`. On startup it loads the file into an in-memory `Map` cache; reads (such as `getOrganizationArray`) are served back from the cache, and writes are saved back to the disk.
* **`ProposalDAO`** --- interface defining the persistence contract (`getProposalById`, `createProposal`, `updateProposal`, `deleteProposal`)
*  **`ProposalFileDAO`** --- `@Repository` implementation that persists proposals to a JSON file, (`data/proposals.json`) using a Jackson `ObjectMapper`. On startup it loads the file into a in-memory `Map` cache; reads (such as `getProposalArray`) are served back from the cache, and writes are saved back to the disk.
...
-   **Controller**
    -   The Controller principle assigns the responsibility of receiving and coordinating a system operation to a class outside of the UI tier. In our codebase this is materialized through the controller folder, which contains each controller file with its own set of purposes. For example, ProposalController contains the rejectedProposal() function which,
        -   It verifies if the manager account is being used before allowing access.
        -   If the manager account is being used, it checks if the input is valid or if one of the error cases happens.
            -   If there is an error nothing happens and an error message is sent.
            -   If the service is successfully completed, it returns an ok status.
    -   This keeps the controller focus only on auth, validation, and request handling, while leaving the business logic to the other files in the backend.
-   **Single responsibility**
    -   Single responsibility states that a class should have only a single responsibility. Our architecture demonstrates this through separation of files and layers:
        -   AccountService and AccountServiceImpl: handles the account-related validation
        -   FundingBasketService and FundingBasketServiceImpl: Exclusively manages basket operations (adding, deleting)
        -   NeedServiceImpl implementing NeedService: handles need creations and sends the required info to the DAO layer to update need.json
    -   For example, NeedService defines the interface for need creation, while NeedServiceImpl , implements the logic and delegates persistence to NeedDAO and NeedFileDAO. Doing this ensures each file has a clear outline purpose and structure that it follows.
-   **Low coupling**
    -   Low coupling attempts to minimize the impact of changes in the system by ensuring each layer has a focused responsibility without depending on internal information from the other layers. Our architecture demonstrates this through the connection between the controller, service, and DAO layers and the data storage tiers.
    -   The Controller layer handles the validation logic and authorization requests. For example it uses isManager() to check if the account is a manager or not. In addition it also sends a HTTP status code, to indicate a success or failure within the code checks and functionality, such as an illegal argument.
    -   The service layer contains the business logic and coordinates operations. Our service interface and their corresponding ServiceImpl files, check for null states, and return values received.
    -   The DAO layer is the persistence layer which handles creating, and updating the values. This layer knows how the data is stored and retrieved.
    -   The data files store information. When a need is created or deleted, the DAO layer edits the JSON file.
...
![CodeCoverageSprint3](Sprint3CodeCoverage.png)

**Sprint 3 Status:** 55% of code covered by unit tests, with the service layer being the most prevalent for missing unit tests. Due to the unexpected roadblocks present in Sprint 3, these tests will be improved upon significantly in Sprint 4.
